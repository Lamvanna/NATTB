...
int main()
{
	...
	hello();
	printf("\nNhap so nguyen duong can tinh giai thua :"); 
	scanf("%d",&h);
	while (h<0 || h>13)
	{
		printf("\nNhap so qua lon roi! Nhap lai n la mot so nguyen duong ="); 
		("%d",&h);
	}
...
//tinh giai thua cua so nguyen duong n
long tinhgiaithua(int n)
{
	int i,x=1;
		if (n==0) return 1;
...
// tinh tong hai so nguyen a va b
int tinhtong (int a, int b)
{
	int tong=0;
	tong=a+b;
